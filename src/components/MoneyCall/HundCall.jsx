    ...
    const handleClaim = async () => {
        ...
        try {
            ...
            if (docSnapshot.exists()) {
                // Document exists, update the 'hundiTimeStamp' field
                await setDoc(manageDocRef, { hundiTimeStamp: serverTimestamp() }, { merge: true });
            } else {
                // Document does not exist, create a new one with 'hundiTimeStamp'
                await setDoc(manageDocRef, { hundiTimeStamp: serverTimestamp() });
            }
        } catch (error) {
    ...
    useEffect(() => {
        const fetchTime = async () => {
            ...
            try {
                ...
                if (docSnapshot.exists()) {
                    const timestamp = docSnapshot.data().hundiTimeStamp?.toMillis();
