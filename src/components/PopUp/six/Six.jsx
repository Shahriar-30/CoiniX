...
function Six(props) {
    ...
    let close = () => {
        return props.value.six(false);
    }


    const handleTwoHun = async () => {
        if (userData === null || userData.amount < 600) {
            ...
            try {
                setMoneyLoad(true);
                await minusAmount(600); // Subtract $200 for investment
                await setDoc(openDocRef, {
                    email: user.email, six: true
                }, { merge: true });
        ...
        <>
            <div className='money_box'>
                <div className='money_m'>
                    <p>
                        Invest <span>$600</span>  to get <br /> <span>$44</span> Every Day
                    </p>
                    <div className='money_btn'>

                        <Button onClick={handleTwoHun}>
                            {moneyLoad ? "Loading..." : "$600"}
                        </Button>
